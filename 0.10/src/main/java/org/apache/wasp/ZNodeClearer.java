...
/**
 * <p>
 * Contains a set of methods for the collaboration between the start/stop
 * scripts and the servers. It allows to delete immediately the znode when the
 * master or the entityGroups server crashes. The entityGroup server / master
 * writes a specific file when it starts / becomes main master. When they end
 * properly, they delete the file.
 * </p>
 * <p>
 * In the script, we check for the existence of these files when the program
 * ends. If they still exist we conclude that the server crashed, likely without
 * deleting their znode. To have a faster recovery we delete immediately the
 * znode.
 * </p>
 * <p>
 * The strategy depends on the server type. For a entityGroup server we store
 * the znode path in the file, and use it to delete it. for a master, as the
 * znode path constant whatever the server, we check its content to make sure
 * that the backup server is not now in charge.
 * </p>
 */
public class ZNodeClearer {
  ...
  public static String getMyEphemeralNodeFileName() {
    return System.getenv().get("WASP_ZNODE_FILE");
  }
