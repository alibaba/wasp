 ...
 * 
 * <p>
 * This is the base class used by trackers in both the Master and fServers.
 */
public abstract class ZooKeeperNodeTracker extends ZooKeeperListener {
  ...
  public synchronized void start() {
    ...
    try {
      ...
      abortable
          .abort("Unexpected exception during initialization, aborting", e);
    }
  }
   ...
   * 
   * @param timeout maximum time to wait for the node data to be available, n
      ...
      if (!nodeExistsChecked) {
        ...
        if (nodeExistsChecked) {
          ...
          try {
            ...
            LOG.warn("Unexpected exception handling blockUntilAvailable", e);
            abortable.abort(
                "Unexpected exception handling blockUntilAvailable", e);
          }
        }
      }
  ...
  public boolean checkIfBaseNodeAvailable() {
    try {
      ...
      abortable.abort("Exception while checking if basenode ("
          + watcher.baseZNode + ") exists in ZooKeeper.", e);
    }
    return true;
  }
}
