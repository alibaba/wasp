...
module Shell
  module Commands
    class Unassign < Command
      def help
        return <<-EOF
Unassign a entityGroup. Unassign will close entityGroup in current location and then
...
Use with caution.  For expert use only.  Examples:
  ...
  hbase> unassign 'ENTITYGROUPNAME'
  hbase> unassign 'ENTITYGROUPNAME', true
EOF
      end

      def command(entityGroup_name, force = 'false')
        format_simple_command do
          admin.unassign(entityGroup_name, force)
        end
      end
    end
  end
