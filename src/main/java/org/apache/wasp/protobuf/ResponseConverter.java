...
import org.apache.wasp.ipc.ServerRpcController;
...
import org.apache.wasp.protobuf.generated.MetaProtos;
...
public final class ResponseConverter {
   ...
   */
  public static org.apache.wasp.fserver.EntityGroupOpeningState getEntityGroupOpeningState(
      final OpenEntityGroupResponse proto) {
    if (proto == null || proto.getOpeningStateCount() != 1)
      return null;
    return org.apache.wasp.fserver.EntityGroupOpeningState.valueOf(proto
   ...
   */
  public static List<org.apache.wasp.fserver.EntityGroupOpeningState> getEntityGroupOpeningStateList(
      final OpenEntityGroupResponse proto) {
    if (proto == null)
      return null;
    List<org.apache.wasp.fserver.EntityGroupOpeningState> entityGroupOpeningStates = new ArrayList<org.apache.wasp.fserver.EntityGroupOpeningState>();
    for (int i = 0; i < proto.getOpeningStateCount(); i++) {
      entityGroupOpeningStates.add(org.apache.wasp.fserver.EntityGroupOpeningState
   ...
   * @return
   */
  public static GetResponse buildGetResponse(Result result,
      List<ColumnStruct> columns) {
    GetResponse.Builder builder = GetResponse.newBuilder();
    boolean existence = checkResultExist(result);
    if (existence) {
      QueryResultProto queryResultProto = ProtobufUtil
      ...
      for (StringDataTypePair StringDataTypePair : StringDataTypePairs) {
   ...
   * @return
   */
  public static InsertResponse buildInsertResponse(OperationStatus status) {
    InsertResponse.Builder builder = InsertResponse.newBuilder();
    WriteResultProto writeResultProto = ProtobufUtil.toWriteResultProto(status);
   ...
   * @return
   */
  public static UpdateResponse buildUpdateResponse(OperationStatus status) {
   ...
   * @return
   */
  public static DeleteResponse buildDeleteResponse(OperationStatus status) {
   ...
   * @return
   */
  public static ExecuteResponse buildExecuteResponse(boolean lastSacn,
      String sessionName,
      Pair<List<QueryResultProto>, List<StringDataTypePair>> queryResults) {
    ExecuteResponse.Builder executeResponseBuilder = ExecuteResponse
        .newBuilder();
    executeResponseBuilder.setSessionId(sessionName);
    for (QueryResultProto queryResultProto : queryResults.getFirst()) {
      ExecuteResultProto.Builder executeResultProtoBuilder = ExecuteResultProto
          .newBuilder();
      executeResultProtoBuilder.setType(ResultType.QUERY);
   ...
   * @return
   */
  public static ExecuteResponse buildExecuteResponse(
      List<WriteResultProto> writeResults) {
    ExecuteResponse.Builder executeResponseBuilder = ExecuteResponse
        .newBuilder();
    for (WriteResultProto writeResultProto : writeResults) {
      ExecuteResultProto.Builder executeResultProtoBuilder = ExecuteResultProto
          .newBuilder();
      executeResultProtoBuilder.setType(ResultType.WRITE);
   ...
   * @return
   */
  public static ExecuteResponse buildExecuteResponse(DropIndexResponse response) {
    return buildExecuteResponse();
  }
   ...
   * @return
   */
  public static ExecuteResponse buildListExecuteResponse(
    ...
    for (int i = 0; i < size; i++) {
      WriteResultProto.Builder writeResultProtoBuilder = WriteResultProto
          .newBuilder();
      writeResultProtoBuilder.setCode(StatusCode.SUCCESS);
      executeResultProtoBuilder.setType(ResultType.WRITE);
  ...
  private static ExecuteResponse buildExecuteResponse() {
    ExecuteResponse.Builder executeResponseBuilder = ExecuteResponse
        .newBuilder();
    ExecuteResultProto.Builder executeResultProtoBuilder = ExecuteResultProto
        .newBuilder();
  ...
  public static ScanResponse buildScanResponse(ScanResponse.Builder builder,
      List<ColumnStruct> columns) {
    List<StringDataTypePair> stringDataTypePairs = buildStringDataTypePairs(columns);
    for (StringDataTypePair StringDataTypePair : stringDataTypePairs) {
      builder.addMeta(StringDataTypePair);
