...
import org.apache.wasp.protobuf.generated.ZooKeeperProtos;
...
public class ZKTable {
  ...
  // TODO: Make it so always a table znode. Put table schema here as well as
  // table state.
  ...
  private void populateTableStates() throws KeeperException {
    synchronized (this.cache) {
      ...
      for (String child : children) {
        ZooKeeperProtos.Table.State state = ZKTableReadOnly.getTableState(
            this.watcher, child);
        if (state != null)
  ...
  private boolean isTableState(final String tableName,
  ...
  public ZooKeeperProtos.Table.State getTableState(final String tableName) {
    synchronized (this.cache) {
  ...
  static Set<String> getAllTables(final ZooKeeperWatcher zkw,
    ...
    for (String child : children) {
      ZooKeeperProtos.Table.State state = ZKTableReadOnly.getTableState(zkw,
          child);
