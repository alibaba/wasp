...
/**
 * Manages the read/write consistency within entityGroup. This provides an
 * interface for readers to determine what entries to ignore, and a mechanism
 * for writers to obtain new write numbers, then "commit" the new writes for
 * readers to read (thus forming atomic transactions).
 */
public class MultiVersionConsistencyControl {
  ...
  private static final ThreadLocal<Long> perThreadReadPoint = new ThreadLocal<Long>() {
    @Override
    ...
    synchronized (writeQueue) {
      if (this.memstoreWrite != this.memstoreRead) {
        throw new RuntimeException(
            "Already used this mvcc. Too late to initialize");
      }
  ...
  /**
   * Set the thread read point to the given value. The thread MVCC is used by
   * the Memstore scanner so it knows which values to skip. Give it a value of 0
