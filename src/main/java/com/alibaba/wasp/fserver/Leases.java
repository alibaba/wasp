 ...
 * Leases
 * 
 * There are several server classes in wasp that need to track external clients
...
public class Leases extends HasThread {
   ...
   * {@link #close()} but rather than violently end all leases, waits first on
   * extant leases to finish. Use this method if the lease holders could loose
   * data, leak locks, etc. Presumes client has shutdown allocation of new
   * leases.
   */
   ...
   */
  public void createLease(String leaseName, int leaseTimeoutPeriod,
      final LeaseListener listener) throws LeaseStillHeldException {
  ...
  public void renewLease(final String leaseName) throws LeaseException {
    synchronized (leaseQueue) {
      ...
      // We need to check to see if the remove is successful as the poll in the
      // run()
   ...
   */
  public Lease removeLease(final String leaseName) throws LeaseException {
    Lease lease = null;
    synchronized (leaseQueue) {
