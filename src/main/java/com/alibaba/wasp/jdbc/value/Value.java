...
public abstract class Value {
   ...
   * class. For some values, compareTo may return 0 even if equals return false.
  ...
  static int getOrder(int type) {
    ...
    case RESULT_SET:
      return 51;
    default:
      throw JdbcException.throwInternalError("type:" + type);
    }
  }
  ...
  public static int getHigherOrder(int t1, int t2) {
    if (t1 == Value.UNKNOWN || t2 == Value.UNKNOWN) {
      if (t1 == t2) {
        ...
        throw JdbcException.get(SQLErrorCode.UNKNOWN_DATA_TYPE_1, "NULL, ?");
   ...
   * 
   * @param v
   *          the value to take the modulus with
   * @return the result
   */
  ...
  public Value convertTo(int targetType) {
    ...
    try {
      ...
      case BOOLEAN: {
        ...
        case JAVA_OBJECT:
        case UUID:
          throw JdbcException.get(SQLErrorCode.DATA_CONVERSION_ERROR_1,
              getString());
        }
        break;
      }
      ...
      case DECIMAL: {
        ...
        switch (getType()) {
        case BOOLEAN:
          return ValueDecimal.get(BigDecimal.valueOf(getBoolean()
              .booleanValue() ? 1 : 0));
        case BYTE:
      ...
      case DATE: {
        ...
        case TIMESTAMP:
          return ValueDate
              .fromDateValue(((ValueTimestamp) this).getDateValue());
        }
        break;
      }
      case TIME: {
      ...
      case TIMESTAMP: {
        switch (getType()) {
        case TIME:
          // return DateTimeUtils.normalizeTimestamp(0,
      ...
      case UUID:
      default:
        throw JdbcException.throwInternalError("type=" + targetType);
      }
    } catch (NumberFormatException e) {
      throw JdbcException.get(SQLErrorCode.DATA_CONVERSION_ERROR_1, e,
          getString());
    }
  }
   ...
   * @param mode
   *          the compare mode
   * @return 0 if both values are equal, -1 if the other value is smaller, and 1
   *         otherwise
   */
  ...
  /**
   * Convert the precision to the requested value. The precision of the returned
   * value may be somewhat larger than requested, because values with a fixed
   * precision are not truncated.
   * 
   * @param precision
   *          the new precision
   * @param force
  ...
  /**
   * Check if this value is linked to a specific table. For values that are kept
  ...
  /**
   * Mark any underlying resource as 'not linked to any table'. For values that
   * are kept fully in memory this method has no effect.
   */
  public void unlink() {
    // nothing to do
  }
  ...
  /**
   * Get a medium size SQL expression for debugging or tracing. If the precision
   * is too large, only a subset of the value is returned.
   * 
   * @return the SQL expression
   */
  ...
  protected JdbcException throwUnsupportedExceptionForType(String op) {
    throw JdbcException.getUnsupportedException(DataType.getDataType(getType())
        .name() + " " + op);
  }
  ...
  public static Value toValue(byte[] values, DataType dataType) {
    switch (dataType) {
    case INT32:
      if(values == null || values.length == 0) {
        return ValueInt.ZERO;
      }
      return ValueInt.get(Bytes.toInt(values));
    case INT64:
      if(values == null || values.length == 0) {
        return ValueLong.ZERO;
      } else if (values.length == 4) {
        return ValueInt.get(Bytes.toInt(values));
      } else {
        return ValueLong.get(Bytes.toLong(values));
      }
    ...
    case DOUBLE:
      ...
      return ValueDouble.get(Bytes.toDouble(values));
    case FLOAT:
      if(values == null || values.length == 0) {
        return ValueFloat.ZERO;
      }
      return ValueFloat.get(Bytes.toFloat(values));
    case STRING:
      return ValueString.get(Bytes.toString(values));
    case DATETIME:
      if(values == null || values.length == 0) {
        return ValueNull.INSTANCE;
      }
      Long ret = Bytes.toLong(values);
      return ValueTimestamp.get(new Timestamp(ret));
    case PROTOBUF:
      // TODO
      return ValueProtobufObject.get(values);
    case UNKNOW:
      return ValueBytes.get(values);
    default:
      throw JdbcException.get(SQLErrorCode.PARSE_ERROR_2,
          "type=" + dataType.name());
    }
