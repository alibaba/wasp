...
/**
 * Callback handler for creating unassigned offline znodes used during bulk
 * assign, async setting entityGroup to offline.
 */
public class OfflineCallback implements StringCallback {
  ...
  OfflineCallback(final ZooKeeperWatcher zkw, final ServerName destination,
      final AtomicInteger counter,
      final Map<String, Integer> offlineNodesVersions) {
    this.callBack = new ExistCallback(destination, counter,
  ...
  /**
   * Callback handler for the exists call that sets watcher on unassigned
   * znodes. Used during bulk assign on startup.
   */
  static class ExistCallback implements StatCallback {
    ...
    ExistCallback(final ServerName destination, final AtomicInteger counter,
        final Map<String, Integer> offlineNodesVersions) {
