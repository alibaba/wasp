...
public class ZKConfig {
  ...
  public static Properties makeZKProps(Configuration conf) {
    ...
    final InputStream inputStream =
      cl.getResourceAsStream(FConstants.ZOOKEEPER_CONFIG_NAME);
    if (inputStream != null) {
      try {
        return parseZooCfg(conf, inputStream);
      } catch (IOException e) {
        LOG.warn("Cannot read " + FConstants.ZOOKEEPER_CONFIG_NAME +
    ...
    if (zkProperties.getProperty(FConstants.CLIENT_PORT_STR) == null) {
      zkProperties.put(FConstants.CLIENT_PORT_STR,
          FConstants.DEFAULT_ZOOKEPER_CLIENT_PORT);
    }
    ...
    final String[] serverHosts = conf.getStrings(FConstants.ZOOKEEPER_QUORUM,
                                                 FConstants.LOCALHOST);
  ...
  public static Properties parseZooCfg(Configuration conf,
    ...
    try {
      ...
      final String msg = "fail to read properties from "
        + FConstants.ZOOKEEPER_CONFIG_NAME;
      LOG.fatal(msg);
    ...
    for (Entry<Object, Object> entry : properties.entrySet()) {
      ...
      if (key.startsWith("server.")) {
        boolean mode = conf.getBoolean(FConstants.CLUSTER_DISTRIBUTED, FConstants.DEFAULT_CLUSTER_DISTRIBUTED);
